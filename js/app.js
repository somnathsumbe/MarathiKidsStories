...
const COLLECTIONS = {
  shivaji: { label: 'शिवाजी महाराज', file: './data/stories.json', count: 150, filters: [['all', 'सर्व कथा'], ['बालपण', 'बालपण'], ['स्वराज्य', 'स्वराज्य'], ['किल्ले', 'किल्ले'], ['पराक्रम', 'पराक्रम'], ['नियोजन', 'नियोजन'], ['प्रशासन', 'प्रशासन'], ['प्रेरणा', 'प्रेरणा']] },
  mahabharat: { label: 'महाभारत', file: './data/mahabharat.json', count: 250, filters: [['all', 'सर्व कथा'], ['महाभारत', 'महाभारत'], ['पांडव', 'पांडव'], ['कौरव', 'कौरव'], ['कुरुवंश', 'कुरुवंश'], ['कुरुक्षेत्र युद्ध', 'कुरुक्षेत्र युद्ध'], ['श्रीकृष्ण', 'श्रीकृष्ण'], ['उपकथा', 'उपकथा']] },
  ramayan: { label: 'रामायण', file: './data/ramayan.json', count: 250, filters: [['all', 'सर्व कथा'], ['बालकांड', 'बालकांड'], ['अयोध्याकांड', 'अयोध्याकांड'], ['अरण्यकांड', 'अरण्यकांड'], ['किष्किंधाकांड', 'किष्किंधाकांड'], ['सुंदरकांड', 'सुंदरकांड'], ['युद्धकांड', 'युद्धकांड'], ['उत्तरकांड', 'उत्तरकांड']] },
  panchatantra: { label: 'पंचतंत्र', file: './data/panchatantra.json', count: 100, filters: [['all', 'सर्व कथा']] },
  isapniti: { label: 'इसापनीती', file: './data/isapniti.json', count: 100, filters: [['all', 'सर्व कथा']] },
  gita: { label: 'गीता अध्याय', file: './data/gita.json', count: 18, filters: [['all', 'सर्व अध्याय'], ['अध्याय १', 'अध्याय १'], ['अध्याय २', 'अध्याय २'], ['अध्याय ३', 'अध्याय ३'], ['अध्याय ४', 'अध्याय ४'], ['अध्याय ५', 'अध्याय ५'], ['अध्याय ६', 'अध्याय ६'], ['अध्याय ७', 'अध्याय ७'], ['अध्याय ८', 'अध्याय ८'], ['अध्याय ९', 'अध्याय ९'], ['अध्याय १०', 'अध्याय १०'], ['अध्याय ११', 'अध्याय ११'], ['अध्याय १२', 'अध्याय १२'], ['अध्याय १३', 'अध्याय १३'], ['अध्याय १४', 'अध्याय १४'], ['अध्याय १५', 'अध्याय १५'], ['अध्याय १६', 'अध्याय १६'], ['अध्याय १७', 'अध्याय १७'], ['अध्याय १८', 'अध्याय १८']] }
};
...
function loadVoices() {
  ...
  voiceMessage = selectedVoice && !hasMarathiVoice ? 'या device/browser मध्ये थेट मराठी आवाज उपलब्ध नाही; उपलब्ध fallback voice वापरला जात आहे.' : (!selectedVoice ? 'या device/browser मध्ये आवाज उपलब्ध नाही.' : '');
...
function speakSegmentV2(index) { if (index >= segments.length) { completeStoryV2(); return; } segmentIndex = index; speechStopped = false; speechPaused = false; saveProgressV2(false); renderReaderV2(currentStoryId, true); const requestId = ++speechRequestId; const start = () => { if (requestId !== speechRequestId || speechStopped) return; utterance = new SpeechSynthesisUtterance(segments[index].text); utterance.lang = 'mr-IN'; if (selectedVoice) utterance.voice = selectedVoice; utterance.rate = SPEEDS[state.audio.speed] || .9; utterance.volume = Number(state.audio.volume ?? 1); utterance.onend = () => { if (requestId === speechRequestId && !speechStopped) speakSegmentV2(index + 1); }; utterance.onerror = (event) => { if (requestId !== speechRequestId || event.error === 'canceled' || event.error === 'interrupted') return; speechStopped = true; speechPaused = false; voiceMessage = 'या device वर मराठी आवाज चालू शकला नाही. पुन्हा ऐका दाबा किंवा recorded audio वापरा.'; renderReaderV2(currentStoryId, true); }; window.speechSynthesis.speak(utterance); window.setTimeout(scrollActiveParagraph, 80); }; if (window.speechSynthesis.speaking || window.speechSynthesis.pending) window.speechSynthesis.cancel(); start(); }
function playStoryV2() { if (!('speechSynthesis' in window)) { voiceMessage = 'या browser मध्ये आवाज सुविधा उपलब्ध नाही. या कथेची recorded audio आवृत्ती उपलब्ध नाही.'; renderReaderV2(currentStoryId, true); return; } loadVoices(); if (!selectedVoice && voices.length === 0) voiceMessage = 'आवाज यादी अजून तयार होत आहे; पुन्हा ऐका दाबल्यावर browser चा default आवाज वापरला जाईल.'; if (!segments.length) prepareSpeechV2(); speakSegmentV2(segmentIndex); }
...
async function showPageV2(page) { stopSpeechV2(false); history.pushState({}, '', page === 'home' ? './' : `#${page}`); if (page === 'home') renderHomeV2(); else { if (!collectionData[activeCollection]) await loadCollectionLazyV2(activeCollection); if (page === 'saved') currentFilter = 'favorites'; renderStoriesV2(); } syncNavigation(); }
function openStoryV2(id, listen = false) { stopSpeechV2(true); collectionComplete = false; currentStoryId = Number(id); state.collection = activeCollection; state.story = currentStoryId; state.playbackMode = playbackMode; saveStateV2(); history.pushState({}, '', `?collection=${activeCollection}&story=${currentStoryId}`); segments = []; renderReaderV2(currentStoryId); if (listen) playStoryV2(); }
...
let collectionError = '';
let listLimitV2 = 30;
let searchDebounceV2 = null;
function renderHomeV2() {
  const cards = COLLECTION_ORDER.map((id) => { const config = COLLECTIONS[id]; const count = config.count; const icon = id === 'shivaji' ? '🚩' : id === 'mahabharat' ? '⚔️' : id === 'ramayan' ? '🏹' : id === 'panchatantra' ? '📚' : id === 'isapniti' ? '🦊' : '🕉️'; return `<button type="button" class="collection-card" onclick="showCollection('${id}')"><span class="collection-card-icon">${icon}</span><span class="collection-card-copy"><strong>${config.label}</strong><small>${count} ${id === 'gita' ? 'अध्याय' : 'कथा'}</small></span><span class="collection-card-action">कथा पाहा →</span></button>`; }).join('');
  const total = Object.values(COLLECTIONS).reduce((sum, config) => sum + config.count, 0);
  document.getElementById('app').innerHTML = `<main class="page-shell home-page"><section class="hero"><div class="hero-copy"><span class="eyebrow">📚 मराठी कथा संग्रह</span><h1>तुमच्या आवडत्या<br><em>कथा आणि अध्याय</em></h1><p>वाचा 📖 • ऐका 🔊 • शिका 🌟</p><div class="hero-stats"><span><strong>${total}</strong> कथा</span><span><strong>🔊</strong> ऐकण्याची सुविधा</span><span><strong>🌟</strong> शिकवण</span></div><button type="button" class="button button-light" onclick="showPageV2('stories')">सर्व कथा वाचा →</button></div><div class="hero-emblem" aria-hidden="true">📖</div></section><section class="collection-section"><span class="section-kicker">कथा संग्रह निवडा</span><h2>आज काय वाचूया?</h2><div class="collection-grid">${cards}</div></section></main>`;
}
function renderStoriesV2() {
  const config = COLLECTIONS[activeCollection];
  if (collectionLoading) { document.getElementById('app').innerHTML = `<main class="page-shell"><section class="empty-state" role="status"><h1>${config.label} लोड होत आहे...</h1><p>कथा तयार होत आहेत.</p></section></main>`; return; }
  if (collectionError || !stories.length) { document.getElementById('app').innerHTML = `<main class="page-shell"><section class="empty-state" role="alert"><h1>${config.label} उघडता आली नाही</h1><p>${collectionError || 'या संग्रहात कथा उपलब्ध नाहीत.'}</p><button type="button" class="button button-primary" onclick="showCollection('${activeCollection}')">पुन्हा प्रयत्न करा</button></section></main>`; return; }
  const filtered = currentFilter === 'favorites' ? stories.filter((story) => isSavedV2(story.id)) : stories.filter((story) => categoryMatches(story, currentFilter) && (!currentSearch || `${story.title} ${story.category}`.toLowerCase().includes(currentSearch.toLowerCase())));
  const list = filtered.slice(0, listLimitV2); const more = filtered.length > list.length;
  document.getElementById('app').innerHTML = `<main class="page-shell stories-page"><header class="page-heading"><div><span class="eyebrow">📚 ${config.label}</span><h1>${config.label}</h1><p>${config.count} ${activeCollection === 'gita' ? 'अध्याय' : 'कथा'} • title वरून शोधा</p></div><button type="button" onclick="toggleTheme()" class="theme-button" aria-label="थीम बदला">${state.theme === 'dark' ? '☀️' : '🌙'}</button></header><div class="collection-tabs">${COLLECTION_ORDER.map((id) => `<button type="button" onclick="showCollection('${id}')" class="filter-button ${activeCollection === id ? 'active' : ''}">${COLLECTIONS[id].label}</button>`).join('')}</div><div class="story-tools"><label class="search-box">🔎<input id="storySearch" placeholder="कथा किंवा अध्याय शोधा..." value="${currentSearch}" aria-label="कथा किंवा अध्याय शोधा"></label><div class="filter-row">${config.filters.map(([id, label]) => `<button type="button" onclick="setFilterV2('${id}')" class="filter-button ${currentFilter === id ? 'active' : ''}">${label}</button>`).join('')}<button type="button" onclick="setFilterV2('favorites')" class="filter-button ${currentFilter === 'favorites' ? 'active' : ''}">♥ आवडत्या</button></div></div><div class="row g-4">${list.map(storyCardV2).join('')}</div>${more ? '<button type="button" class="button button-outline load-more-button" onclick="loadMoreV2()">आणखी कथा दाखवा</button>' : ''}${!list.length ? '<div class="empty-state">जुळणाऱ्या कथा नाहीत.</div>' : ''}</main>`;
  document.getElementById('storySearch')?.addEventListener('input', (event) => { window.clearTimeout(searchDebounceV2); searchDebounceV2 = window.setTimeout(() => { currentSearch = event.target.value; listLimitV2 = 30; renderStoriesV2(); }, 250); });
}
function loadMoreV2() { listLimitV2 += 30; renderStoriesV2(); }
async function loadCollectionLazyV2(collectionId) { if (collectionData[collectionId]) { loadCollectionV2(collectionId); return true; } collectionLoading = true; collectionError = ''; activeCollection = collectionId; renderStoriesV2(); try { const response = await fetch(COLLECTIONS[collectionId].file); if (!response.ok) throw new Error('fetch failed'); const loaded = await response.json(); if (!Array.isArray(loaded)) throw new Error('invalid data'); collectionData[collectionId] = loaded.filter((story, index, list) => list.findIndex((item) => Number(item.id) === Number(story.id)) === index); loadCollectionV2(collectionId); collectionLoading = false; renderStoriesV2(); return true; } catch { collectionLoading = false; collectionError = 'Internet connection तपासा आणि पुन्हा प्रयत्न करा.'; stories = []; renderStoriesV2(); return false; } }
async function showCollectionV2(collection) { if (!COLLECTIONS[collection]) return; stopSpeechV2(false); currentFilter = 'all'; currentSearch = ''; listLimitV2 = 30; state.collection = collection; state.story = COLLECTIONS[collection].count ? 1 : 0; saveStateV2(); history.pushState({}, '', '#stories'); await loadCollectionLazyV2(collection); syncNavigation(); }
async function initializeV2() { state = migrateStateV2(); activeCollection = state.collection || 'shivaji'; playbackMode = state.playbackMode || 'single'; loadVoices(); const params = new URLSearchParams(location.search); if (params.get('story') || location.hash === '#stories' || location.hash === '#saved') { await loadCollectionLazyV2(activeCollection); renderCurrentV2(); } else renderHomeV2(); ensureCollectionStartActionsV2(); syncNavigation(); }
