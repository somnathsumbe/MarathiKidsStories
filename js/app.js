...
function renderHomeV2() {
  const cards = COLLECTION_ORDER.map((id) => { const config = COLLECTIONS[id]; const count = config.count; return `<button type="button" class="collection-card" onclick="showCollection('${id}')"><span class="collection-card-icon">${id === 'shivaji' ? '🚩' : id === 'mahabharat' ? '⚔️' : id === 'ramayan' ? '🏹' : '🕉️'}</span><span class="collection-card-copy"><strong>${config.label}</strong><small>${count} ${id === 'gita' ? 'अध्याय' : 'कथा'}</small></span><span class="collection-card-action">कथा पाहा →</span></button>`; }).join('');
  const total = Object.values(COLLECTIONS).reduce((sum, config) => sum + config.count, 0);
  document.getElementById('app').innerHTML = `<main class="page-shell home-page"><section class="hero"><div class="hero-copy"><span class="eyebrow">📚 मराठी कथा संग्रह</span><h1>तुमच्या आवडत्या<br><em>कथा आणि अध्याय</em></h1><p>वाचा 📖 • ऐका 🔊 • शिका 🌟</p><div class="hero-stats"><span><strong>${total}</strong> कथा</span><span><strong>🔊</strong> ऐकण्याची सुविधा</span><span><strong>🌟</strong> शिकवण</span></div></div><div class="hero-emblem" aria-hidden="true">📖</div></section><section class="collection-section"><span class="section-kicker">कथा संग्रह निवडा</span><h2>आज काय वाचूया?</h2><div class="collection-grid">${cards}</div></section></main>`;
}
function renderStoriesV2() {
